...
import { saveSpotTag, deleteSpotTag } from "@/lib/actions";
import type { AdminTag } from "@/lib/admin";

export default function SpotTagRow({ tag }: { tag: AdminTag }) {
    ...
    return (
      <form
        ...
        action={(fd) => {
          ...
          start(async () => {
            await saveSpotTag(tag.id, name.trim(), sort);
            setEditing(false);
          });
        }}
      >
  ...
  return (
    <div className="row" style={{ display: "flex", alignItems: "center", gap: 12 }}>
      ...
      <button
        ...
        disabled={pending}
        onClick={() => {
          if (confirm(`Delete "${tag.name}"?`)) start(() => deleteSpotTag(tag.id));
        }}
      >
        Delete
      </button>
