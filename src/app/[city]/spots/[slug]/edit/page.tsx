...
export default async function EditSpotPage({ params }: { params: Promise<{ city: string; slug: string }> }) {
  ...
  if (!isCreator && !isAdmin) redirect(`/${city}/spots/${slug}`);

  const tags = await listSpotTags();
  return (
    <div>
      <Link className="back" href={`/${city}/spots/${slug}`}>
        ‹ cancel
      </Link>
      <SpotForm
        tags={tags}
