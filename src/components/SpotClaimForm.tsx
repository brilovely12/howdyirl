...
import { submitSpotClaim } from "@/lib/actions";

export default function SpotClaimForm({ spotId }: { spotId: string }) {
  ...
  return (
    <div style={{ border: "1px solid var(--rule)", borderRadius: 3, padding: 14, marginTop: 4 }}>
      <div style={{ fontWeight: 600, marginBottom: 8 }}>Claim this spot</div>
      ...
      />
      <label style={{ fontSize: 12, color: "var(--ink-dim)" }}>How are you connected to this spot? (optional)</label>
      <textarea
        value={note}
        onChange={(e) => setNote(e.target.value)}
        rows={2}
        placeholder="e.g. I'm the owner"
        style={{
          width: "100%",
          background: "var(--panel-2)",
          color: "var(--ink)",
        ...
        <button
          ...
          onClick={() => {
            startTransition(async () => {
              await submitSpotClaim(spotId, email, note);
              setDone(true);
            });
          }}
        >
